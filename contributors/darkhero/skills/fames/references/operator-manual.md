# FAMES operator reference

Cold reference: read only a relevant section. The current SKILL.md controls activation and generation. This reference preserves detailed workflows; it is never injected wholesale.

Treat FAMES as one completion contract, not as a label or permission grant. Load only the protocol section needed for the current acceptance gap and run `token-preflight` before broad exploration. The bundled protocol is the portable authority; if `%AI_WORKSPACE%\_registry\fames-protocol.json` also exists, require it to describe the same version and execution order or report `UNKNOWN`.
...
contract and the relevant section. See [minimal context](minimal-context.md)
...
current protocol and [the shared job contract](autonomous-knowledge.md).
...
knowledge item, read [knowledge evidence](knowledge-evidence.md).
...
[the production delivery profile](production-web-delivery.json). Its entity,
...
Load the [hardware compute profile](hardware-compute.json) before assigning
...
## Auto goal-compact — the goal vector survives every compaction

`auto_goal_compact` is a lane, not a sixth phase (protocol section `auto_goal_compact`). When a seat's context is compacted (AGC `agc_should_compact`, a phase boundary, a milestone park, the platform's own pre-compaction hook, or a manual run), `scripts/fames_fleet.py goal-compact --workspace <hub> --agent <seat> --trigger <trigger> --json` projects `_registry/agc-compact/<seat>.md` onto its goal vector. Every `## ` block is classed by heading: goal-parallel classes (goal vector, PARK working set, stash pointers, PFKT pending, recent SUBMIT, red lines, invariants, terminal states, artifact paths) are copied byte-identical; goal-orthogonal classes (recent compaction notes, session signals, raw tool output, exploratory reads, superseded drafts, chatter, completed segment transcripts) move to `_registry/agc-compact/<seat>.orthogonal.md` and leave one `- receipt:` line behind. Nothing is summarised: the projection is deterministic, costs zero tokens, and every receipt is openable in the ledger by sha256.

The goal identity is resolved before and after the projection (explicit `--goal` file, then `_logs/<stamp>.goal.json` named by the goal-vector stamp, then the compact's own goal block) and both hashes must equal `semantic_goal_hash(goal)`. A block that carries an operator red line (`RL-*`) or an `UNKNOWN`/`FORBIDDEN` terminal state is never compressed even when its heading is orthogonal (RED_LINE_PRECEDENCE). Unclassified headings are kept verbatim and charged as residue: `compact_orth_penalty = residue_bytes / bytes_after` and `P_compact = 1 - compact_orth_penalty`, the compaction term the goal-vector protocol already defines.

The lane fails closed. The record is validated by `validate-goal-compact` before any byte is written; hash drift, authority expansion, a dropped red line, an undeclared class, a fabricated line, non-zero quota, an undeclared trigger, an unparsable timestamp or a forbidden-material key make the run UNKNOWN and nothing is written. After a write the compact is re-read and its sha must equal `result_identity`, otherwise no evidence is emitted. Evidence lands in `_registry/fames-evidence/agc-<stamp>-<seat>.json` for each distinct projection and `agc-<seat>-latest.json` on every run; SEAL requires the latest record with `goal_hash_matches`, `red_lines_preserved` and `fabricated_lines_count == 0` for any completion claim made after a compaction.

Wiring: `_skill/engines/agc_lib.py write_compact` calls the engine best effort (hidden window, 30 s timeout) and stores the outcome as `fames_goal_compact` in the AGC state file, so every AGC compaction that goes through `write_compact` (the beforeSubmitPrompt refresh, `agc-compact.py`, `goal_compact.py park`) is covered without a new rider. The Cursor `preCompact` entry (`compaction-guard.ps1`, generated by `sync-cursor-hooks.py`) stays the recorder there. On Claude Code the lane is wired into every hook by `_harness/runtime/fames_session_harness.py` (function `auto_goal_compact`, lane id `FAMES-AGC-LANE`): `SessionStart` mints or refreshes the compact and appends a short pointer to the session receipt `_registry/fames-session/<seat>.json` (check token `AGC-LANE-SESSION-START`; after a platform compaction, `source=compact`, the pointer carries `AGC-LANE-POST-COMPACTION-POINTER` and orders resumption from the compact rather than the platform summary); `UserPromptSubmit` evaluates `agc-should-compact` and refreshes silently when a limit is crossed (`AGC-LANE-TURN`, receipt under `_registry/fames-turn/`); `PreCompact` runs the armed adapter `_skill/engines/claude-agc-precompact-hook.py`, which forces a `platform_pre_compaction` refresh, ledgers the compaction and writes `_registry/fames-precompact/<seat>.json` (`AGC-LANE-PRECOMPACT`). The seat is resolved from the transcript slug, then the hook agent, then the seat-root name; an unresolved seat is UNKNOWN and writes nothing. Probes (`fames_probe_mode`) exercise the lane but never append ledger rows. No HubClock rider is armed. Cases `C-GOALCOMPACT-*` in `references/cases.json` pin the validator and `C-AGC-*` pin the wiring: every `hooks.claude` row flagged `armed` in `_registry/agc-protocol.json` must name a check token that exists in the harness source.
...
It runs `status` and then every case declared in [references/cases.json](cases.json), and writes a replayable record to `_registry/fames-evidence/self/<stamp>.json`. No model is called and no quota is spent, so it is admissible `measured` evidence and may be run as often as it is useful.
