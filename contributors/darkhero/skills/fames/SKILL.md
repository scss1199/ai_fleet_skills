...
Wiring: `_skill/engines/agc_lib.py write_compact` calls the engine best effort (hidden window, 30 s timeout) and stores the outcome as `fames_goal_compact` in the AGC state file, so every AGC compaction that goes through `write_compact` (the beforeSubmitPrompt refresh, `agc-compact.py`, `goal_compact.py park`) is covered without a new rider. The Cursor `preCompact` entry (`compaction-guard.ps1`, generated by `sync-cursor-hooks.py`) stays the recorder. A Claude-side PreCompact hook running `--trigger platform_pre_compaction` is a documented proposal only; it is not armed because it needs an operator-approved edit of the user-level settings file. Cases `C-GOALCOMPACT-*` in `references/cases.json` pin the validator.
