...
The shared gate rejects recognized proof wording without a matching audited theorem.
This applies where the caller actually invokes the gate; installed configuration alone does
not establish every host's runtime coverage. To satisfy it, run `attest`, copy the exact
corresponding `claim_lines` entry and its `evidence_lines` marker. An arbitrary prose claim
with a valid marker is refused: a proof of `True` does not establish an unrelated assertion.

```
python C:/ai_workspace/_lean/fames/lean_gate.py attest --file proof.lean
# -> claim_lines plus evidence_lines (copy the matching pair without paraphrasing)
```

The gate replays that attestation; a forged path, a flipped sha, or a wrong theorem name is
rejected. Canonical claims retain typed numerals, dependency names and audit cautions. Inspect
the linked source for local definition bodies; names alone do not establish intended meaning.
Attestation and replay schema `/2` requires older proofs to be re-attested. Checker identity
includes executable/DLL hashes and dependency metadata; compiled import bytes and arbitrary
prose equivalence are not attested. If you could not prove it, state it as UNKNOWN.
`evidence_ok` in the
